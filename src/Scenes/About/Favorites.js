...
class Favorites extends Component {
  render() {
    const keys = [];
    Object.keys(favs).forEach(function(key, i) {
      const values = [];
      favs[key].forEach(function(val, i) {
        // Only include a comma if it's not the last element
        const comma = i === favs[key].length - 1 ? '' : ', ';
        const value = <span key={val}>
      ...
      const comp = <p key={key} style={{display:'block'}}>
